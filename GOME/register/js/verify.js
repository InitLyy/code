...
$('#label').mouseup(function(){
	isMove=false;
	var cur=parseInt($('#label').css('left'));
	if(cur==252){
		ok=true;
	}
	else{
...
$('#label').mousemove(function(e){
	if(isMove==true){
		var now=parseInt(e.pageX);
		$('#label').css('left',(now-pre)+'px');
		$('#slider_bg').css('width',(now-pre)+'px');
	}
	var cur=parseInt($('#label').css('left'));
	if(cur>=252&&isMove==true){
		$('#label').css('left','252px');
		$('#label').css('background','#FFFFFF url(image/register_slider.png) no-repeat scroll 50% 50%');
		$('#label').text("");
		$('#slider_bg').css('width','252px');
		$('#labelTip').text('验证通过');
		isMove=false;
		judge=true;
	}
...
$('#registerNow').click(function(){
	//alert(flag);
	var flag=0;
	if($('#nameSuc').css('display')=='block')
		flag+=1;
	if($('#passwordSuc').css('display')=='block')
		flag+=1;
	if($('#confirmPasswordSuc').css('display')=='block')
		flag+=1;
	if($('#checkMobileSucc').css('display')=='block')
		flag+=1;
	if($('#checkSuc').css('display')=='block')
		flag+=1;
	
	if(flag==5&&judge==true){
		var info=new Array();
